...
### 序列化
85. 其他方法优先于 Java 序列化  
 1）序列化的攻击面非常庞大，无法进行防护，并且它还在不断扩大。其对象是通过在 ObjectInputStream 上调用 readObject 进行反序列化的。这个方法其实是个神奇的构造器，它可以将类路径上几乎任何类型的对象都实例化，只要该类型实现了 Serializable 接口。在反序列化字节流的过程中，该方法可以执行任意类型的代码，因此所有这些类型的代码都是攻击的一部分。  
 2）避免序列化攻击的最佳方式是永远不要反序列化任何东西；  
 3）可以用跨平台结构化数据表示法 JSON 来代替 Java 序列化；  
 4）永远不要反序列化不被信任的数据
86. 谨慎实现 Serializable 接口  
 1）可序列化类的参数 serialVersionUID（私有、静态、final）如果不手动指定，而是依靠自动生成，则后面改变或扩展这个类的时候会改变 serialVersionUID 这个值，从而破坏兼容性；  
 2）序列化机制是有一种语言之外的对象创建机制；  
 3）依靠默认的反序列化机制，很容易使对象的约束关系遭到破坏，以及遭受到非法访问；  
 4）为了继承而设计的类，应该尽可能少地去实现 Serializable 接口，用户的接口也应该尽可能少地继承 Serializable 接口，当然这条规则也不绝对  
87. 考虑使用自定义的序列化形式  
 1）如果事先没有认真考虑默认的序列化形式是否合适，则不要贸然接受；  
 2）如果一个对象的物理表示法等同于它的逻辑内容，可能就适合于使用默认额的序列化形式；  
 3）即使确定了默认的序列化形式是合适的，通常还必须提供一个 readObject 方法以保证约束关系和安全性；  
 4）无论是否使用默认的序列化形式，当 defaultWriteObject 方法被调用的时候，每一个未被标记为 transient 的实例域都会被实例化；而对于标记为 transient 的域，则会在反序列化的时候初始化为默认值  
88. 保护性地编写 readObject 方法  
 1）readObject 相当于“以字节流作为唯一参数”的公有构造器，它可以用人工伪造的字节流来创造普通构造器无法创建出来的“不可能的”对象；  
 2）当一个对象被反序列化的时候，对于客户端不应该拥有的对象引用，如果哪个域包含了这样的对象引用，就必须要做保护性拷贝，这是非常重要的；  
 3）因此，对于每个可序列化的不可变类，如果它包含了私有的可变组件，那么在它的 readObject 方法中，必须要对这些组件进行保护性拷贝，这可以解决（1）中的问题；  
 4）readObject 方法不可以调用可被覆盖的方法，无论是直接调用还是间接调用都不可以，如果违反了这条规则，被覆盖的方法将在子类的状态被反序列化前运行，那么很可能会导致失败
89. 对于实例控制，枚举类型优先于 readResolve  
 1）任何一个 readObject 方法，不管是显式的还是默认的，都会返回一个新建的实例，这个新建的实例不同于该类初始化时创建的实例；  
 2）readResolve 对象允许我们用 readObject 创建的实例代替另一个实例，可以解决（1）中的问题；  
 3）如果依赖 readResolve 进行实例控制，带有对象引用类型的所有实例域则必须声明为 transient；  
 4）如果将一个可序列化的实例受控的类编写成枚举，Java 就可以绝对保证除了所声明的常量之外，不会有其他实例  
90. 考虑用序列化代理代替序列化实例  
 1）首先，为可序列化的类设计一个私有的静态嵌套类，精确地表示一个私有的外围类的逻辑状态。这个嵌套类被称作序列化代理，
它应该有一个单独的构造器，其参数类型就是那个外围类。这个构造器只从它的参数中复制数据，它不需要进行任何一致性检查或者保护性拷贝。
而且这个外围类及其序列代理都必须声明实现 Serializable 接口；  
 2）序列化代理模式有两个局限性：第一，它不能与可以被客户端扩展的类相兼容，也不能与对象图中包含循环的某些类相兼容；第二，它比使用保护性拷贝的安全措施增加了时间上的开销  